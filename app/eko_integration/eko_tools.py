...
class EkoToolResult:
    """Eko工具执行结果"""
    def __init__(self, success: bool = True, result: Any = None, error: Optional[str] = None, metadata: Dict[str, Any] = None):
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url, params=params) as response:
                    ...
                    if data.get("status") == "1" and data.get("geocodes"):
                        ...
                        return EkoToolResult(success=False, error=f"地理编码失败: {data.get('info', '未知错误')}")
