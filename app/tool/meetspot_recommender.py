...
"""
    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        keywords = kwargs.get('keywords', '咖啡馆')
        place_type = kwargs.get('place_type', '')
        user_requirements = kwargs.get('user_requirements', '')
        theme = kwargs.get('theme', '')  # 添加主题参数
        
        # 参数验证
        if not locations:
            return ToolResult(output="推荐失败: 缺少locations参数")
        
        if not isinstance(locations, list):
            return ToolResult(output="推荐失败: locations参数必须是列表")
...
<head>
    ...
    <title>{cfg["topic"]} - 最佳会面{cfg["noun_singular"]}推荐 | EkoMeet</title>
...
<body>
    ...
    <footer class="footer">
        <div class="container">
            <p>© {datetime.now().year} {cfg["topic"]} - 基于Eko AI的智能{cfg["noun_singular"]}推荐服务 | 数据来源：高德地图</p>
        </div>
    </footer>
    ...
    def _generate_search_process(
        ...
        <div class="card search-process-card">
            <h2 class="section-title"><i class='bx bx-bot'></i>Eko搜索过程</h2>
